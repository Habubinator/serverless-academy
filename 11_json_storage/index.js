...
app.put('/:json_path', (req, res) => {
  ...
  fs.writeFile(filePath, JSON.stringify(jsonDocument, null, 2), (err) => {
    if (err) {
      return res.status(500).json({ error: 'Failed to store JSON document' });
    }
    res.status(201).json({ message: 'JSON document stored successfully' });
  });
});
...
app.get('/:json_path', (req, res) => {
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    if (err) {
      return res.status(404).json({ error: 'JSON document not found' });
    }
